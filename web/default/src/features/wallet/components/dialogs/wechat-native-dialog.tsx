...
interface WechatNativeDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  order: WechatNativeOrder | null
  refreshing: boolean
  refreshError: boolean
  onRefresh: () => void
}

export function WechatNativeDialog(props: WechatNativeDialogProps) {
  ...
  } else if (props.order?.code_url) {
    paymentContent = (
      <div className='flex flex-col items-center gap-4'>
        <div className='rounded-md border bg-white p-3'>
          <QRCodeSVG
            value={props.order.code_url}
            size={224}
            level='M'
            marginSize={0}
  ...
  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent className='max-sm:w-[calc(100vw-1.5rem)] sm:max-w-sm'>
        <DialogHeader>
          <DialogTitle>{t('Scan with WeChat')}</DialogTitle>
          <DialogDescription>
