...
## 2026-07-23 实时复验与范围收敛

- DeepKey 实时公开目录现为 235 个模型、44 个倍率分组、45 个分组说明、7 个供应商和 10 类端点；本地客户分组与上游差异仅包含 `default/test` 及已隔离分组。
- 42 把 DeepKey 上游 Key 的 `/v1/models` 再次全部成功，授权模型并集为 235 个；本地渠道没有发布任何不在对应 Key 授权清单中的模型，渠道模型与 521 条启用能力记录一致。
- `gemini-t3` 的 Key 仍在 `/v1/models` 中声明 `gemini-2.5-flash-lite`，但真实请求返回 404“新用户不可用”；已从该渠道及能力清单移除，改用 `gemini-2.5-flash` 后客户调用成功。这证明模型列表只能作为候选，不能代替真实请求验收。
- `claude-aws-bedrock` 使用客户密钥连续三次重试仍返回 DeepKey 429“模型暂时不可用”，已手动禁用并从客户可见分组移除。当前隔离分组增至 5 个：`claude-ai`、`claude-aws-platform`、`claude-aws-bedrock`、`claude-max-sale`、`claude-vertex`。
- 当前生产可售范围为 37 个分组，37 个启用渠道使用 37 把不同 DeepKey 上游 Key。逐分组创建 37 把不同客户随机 Key 后，36 个在全量轮次直接成功；`codex-k12` 因上游“待处理请求过多”瞬时返回 429，独立复试恢复并成功，最终 37/37 均通过真实客户调用。
- 37 个成功分组均命中预期 `channel_id`，消费日志均写入对应客户 `user_id`、本地 `token_id`、分组、模型、输入/输出 Token 和实际 `quota`。失败的 404、429 和参数校验请求没有生成消费日志，也没有扣减客户额度。
