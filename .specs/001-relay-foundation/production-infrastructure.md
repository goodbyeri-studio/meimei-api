...
生产机的 `/etc/meimei-api/production.env` 不进入仓库或 Actions 日志。微信支付 Variables/Secrets 由 production workflow 校验后，完整暂存到 `/etc/meimei-api/secrets/wechatpay/releases/<release-id>`，每个版本包含合并后的环境文件和 PEM；容器只读挂载发布清单指定的版本目录，流水线不输出密钥内容。`deploy/production/deploy.sh` 的 `current.env` 与 `previous.env` 同时固定镜像 digest、环境文件和 Secret 版本，部署健康检查失败或执行 `rollback` 时一并恢复，避免支付配置与镜像版本错配。远端上传 bundle 无论安装、上传或后续步骤成功失败都会清理。GitHub `production` Environment 的部署凭据、服务器 deploy 用户和 Registry 只读凭据已经配置并完成首次发布验证；数据库备份恢复流程仍需演练。
