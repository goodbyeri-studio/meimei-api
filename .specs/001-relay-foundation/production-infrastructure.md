...
当前是“基础设施已按单 App 方案收敛、应用尚未发布”状态：Reserved IP、DNS、Docker、Caddy 和 Let's Encrypt TLS 已完成，但 App Droplet 仍未运行 MeiMei API，production Secret、固定 SHA 镜像、模型渠道和真实流量压测尚未完成。因此不能把当前资源状态描述为 MeiMei API 已经上线。
...
生产机的 `/etc/meimei-api/production.env` 不进入仓库或 Actions 日志。`deploy/production/deploy.sh` 在拉取候选 digest 后才切换 `current.env`，部署健康检查失败时恢复 `previous.env`；`rollback` 可以直接复用上一版本，不重新构建镜像。首次上线前仍需创建 GitHub `production` Environment 的部署凭据、服务器 deploy 用户、Registry 只读凭据和备份/恢复流程。
