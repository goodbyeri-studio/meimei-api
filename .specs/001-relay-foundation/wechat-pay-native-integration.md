...
> 状态：设计与实施依据
> 适用范围：莓莓 API 的普通商户模式（商户自有 `mchid`）微信支付 Native 扫码支付
> 最后核对：2026-07-22
...
4. `description` 不超过 127 个字符，应如实描述，例如“莓莓 API 额度充值（100 元）”。不要记录用户邮箱、手机号、模型提示词、API key 或业务敏感数据。
