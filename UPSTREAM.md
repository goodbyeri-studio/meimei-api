...
```text
origin   https://github.com/goodbyeri-studio/blackrain-relay.git
upstream https://github.com/QuantumNous/new-api.git
```

`origin/main` 是 BlackRain Relay 可发布基线；`upstream` 只用于评估和同步 New API。
...
3. 在短分支上把目标 tag 合入或重放 BlackRain patch；禁止直接跟随 `upstream/main`。
4. 解决冲突时优先保留上游品牌、署名、License 和安全修复；BlackRain 改动应尽量位于独立文件或稳定扩展边界。
5. 跑上游 Go、前端、数据库兼容、billing、relay protocol 和 migration 测试。
6. 跑 BlackRain Cloud token/usage 合同、WORK Chat、CODE Responses、限流、余额和故障矩阵。
...
- 把上游测试数量当作 BlackRain Cloud/Relay/Desktop E2E 已通过。
