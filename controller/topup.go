...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		topUp, quotaToAdd, credited, completeErr := model.CompleteEpayTopUp(verifyInfo.ServiceTradeNo, verifyInfo.Type)
		if completeErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 充值入账失败 trade_no=%s callback_type=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), completeErr.Error()))
			return
		}
		if credited {
